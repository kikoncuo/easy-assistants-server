import { StateGraph, CompiledStateGraph, StateGraphArgs, START, END } from '@langchain/langgraph';
import { AbstractGraph, BaseState } from './baseGraph';
import { authenticate, getSchema, executeQuery, createCard } from '../utils/MetabaseAPI';  
import { createStructuredResponseAgent, getStrongestModel, anthropicSonnet } from '../models/Models';
import { HumanMessage } from '@langchain/core/messages';
import { ToolDefinition } from '@langchain/core/language_models/base';
import Logger from '../utils/Logger';


interface MetabaseState extends BaseState {
  sessionToken: string;
  schema: any[];
  cardId: number;
  queryResult: any;
...
async function authenticateMetabase(state: MetabaseState): Promise<MetabaseState> {
  const sessionToken = await authenticate(); // TODO: Move this to the same step as fetching the schema
  return {
    ...state,
    sessionToken,
  };
...
async function fetchSchema(state: MetabaseState): Promise<MetabaseState> {
  const schema = await getSchema(state.sessionToken, 1);  // TODO: Here we assume that the database_id is 1, but this should be dynamic
  return {
    ...state,
    schema,
  };
...
async function createMetabaseCard(state: MetabaseState): Promise<MetabaseState> {
  ...
  if (typeof cardIdResponse === 'object' && 'error' in cardIdResponse) {
    throw new Error(`Failed to create card: ${cardIdResponse.error} (Status: ${cardIdResponse.status})`); // By throwing the error we stop the subgraph and properly handle the error showing the correct message unlike before
  }

  const cardId = cardIdResponse;
...
async function executeMetabaseQuery(state: MetabaseState): Promise<MetabaseState> {
  const queryResult = await executeQuery(state.sessionToken, state.cardId); // TODO: Check if the result is correct and makes sense or it needs to be re-executed
  return {
    ...state,
    queryResult,
    finalResult: state.finalResult || JSON.stringify(queryResult, null, 2),  // In case it's set before because of an error we don't overwrite it. TODO: go to END if there's an error on the earlier node
  };
}

export class DataRecoveryGraph extends AbstractGraph<MetabaseState> {
  constructor() {
    const graphState: StateGraphArgs<MetabaseState>['channels'] = {
      sessionToken: {
  ...
  getGraph(): CompiledStateGraph<MetabaseState> {
    const subGraphBuilder = new StateGraph<MetabaseState>({ channels: this.channels });

    subGraphBuilder
      .addNode('authenticate', async state => await authenticateMetabase(state))
      .addNode('fetch_schema', async state => await fetchSchema(state))
      .addNode('create_card', async state => await createMetabaseCard(state))
      .addNode('execute_query', async state => await executeMetabaseQuery(state))
      .addEdge(START, 'authenticate')
      .addEdge('authenticate', 'fetch_schema')
      .addEdge('fetch_schema', 'create_card')
      .addEdge('create_card', 'execute_query')
      .addEdge('execute_query', END);
