...
export class GraphApplication {
  ...
  constructor(outputHandler: (type: string, message: string) => void = Logger.log, agentFunction: Function) {
    ...
    const agents = {
      ...
      getTables: {
        agent: createAgent(llama8bGroq, [getTables]),
      ...
      organizeItems: {
        agent: createAgent(llama8bGroq, [organizeItems]),
    ...
    this.graphManager = new GraphManager(createPlanner(strongestModel), agents, llama8bGroq, outputHandler, agentFunction);
  }
