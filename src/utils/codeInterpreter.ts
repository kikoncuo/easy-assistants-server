import { CodeInterpreter, ProcessMessage } from '@e2b/code-interpreter'
    ...
    code,
    {
      onStdout: (msg: ProcessMessage) => {
        ...
        if (functions && functions.length > 0) {
          functions[0]('stdout', [{ function_name: 'logStdout', arguments: { message: msg.line } }]);
        }
      },
      onStderr: (msg: ProcessMessage) => {
        console.log("\n[Code Interpreter stderr]", msg);
        if (functions && functions.length > 0) {
          functions[0]('stderr', [{ function_name: 'logStderr', arguments: { message: msg.line } }]);
        }
      },
    }
  )
