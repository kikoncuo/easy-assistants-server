...
Here are the tools you have access to:
    ...
    getData: Use this tool exclusively when a user requests something that requires data extraction in any format table or graph. Input is an array of descriptions of the data that needs to be retrieved and what de data is for, you can provide multiple descriptions calling this tool once. You can only call this once, but you can request multiple things in the same query. The element to be defined in the displayType is based on user's input task. (IE: Craete a table, displayType should be table, create a bar graph, displayType should be barChart.) 
