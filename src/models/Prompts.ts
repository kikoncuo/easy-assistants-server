...
Here are the tools you have access to:
    ...
    shippingAlertAgent: Use this tool to generate cron expressions and SQL queries for scheduling alerts. You will always respond with cron expressions, SQL queries, and other relevant information based on user inputs for alerting when products require shipping.
    triggerNotification: Use this tool to insert data into the database. You will always respond with the sql query to insert value into the database.
...
Example 4: if the user were to give the task: Alert me whenever products require shipping, every 2 minutes
We would create a plan with 2 steps, #E1 and #E2:
#E1 would call the shippingAlertAgent tool with parameters ["2 minutes", "SELECT name FROM notifications.products WHERE shipping_required = true", "COUNT(*) > 0", "These products need to be shipped", "alert", "Sends an alert when products require shipping"] and describe the step as "organized data for creating a shipping alert"
#E2 would call the triggerNotification tool with parameters ["#E1"] and describe the step as "Analyze the provided dataset and create a SQL query to insert data to the database."
