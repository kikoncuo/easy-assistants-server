...
export const IdentifyFieldsTool: ToolDefinition = {
  ...
  function: {
    name: 'identifyFields',
    description:
      'Identify fields in the schema that require additional details for the query and determine the difficulty of calculating the necessary values with the current fields.',
    parameters: {
      type: 'object',
      properties: {
        fieldIds: {
    ...
    parameters: {
      ...
      properties: {
        ...
        collection_id: {
          type: 'integer',
          description:
            'The ID of the collection where the card will be stored, for now pick 2 as is the default library folder', // TODO: Keep track of this in case we use multiple folders
          minimum: 1,
        },
        name: {
          type: 'string',
        ...
        dataset_query: {
          ...
          properties: {
            ...
            query: {
              type: 'object',
              description:
                'The query structure, including filters, aggregations, etc. Remember to use the CubeJoinField fields that all tables have to as source tables and identify the fields by their IDs',
            },
          },
    ...
    parameters: {
      ...
      properties: {
        ...
        semanticTask: {
          type: 'array',
          description:
            'List of field names to be added to the model of the semantic layer if needed. Return the list of value with this format: cube_name.property (e.g product.storeRevenue).',
          items: {
            type: 'object',
            properties: {
              value: { type: 'string' },
...
export const TableIdentifyingTool: ToolDefinition = {
  ...
  function: {
    ...
    description:
      'Identify the most relevant tables in the schema for the given task, considering relationships between tables and choosing only the necessary tables to answer the task efficiently.',
    parameters: {
      type: 'object',
      properties: {
        reasoning: {
          type: 'string',
          description:
            'A brief explanation of why these tables were chosen and how they relate to the task. Always mention the name of the table explicitly.',
        },
        relevantTables: {
          type: 'array',
          description:
            'An array of objects representing the relevant tables. (Do not use fields, datapoints or variables, only tables)',
          items: {
            type: 'object',
            properties: {
              id: {
...
export const GeneratePlanTool: ToolDefinition = {
  ...
  function: {
    name: 'generatePlan',
    description:
      'Generates a detailed step-by-step plan for extracting insights from the table data based on the given task.',
    parameters: {
      type: 'object',
      properties: {
        plan: {
