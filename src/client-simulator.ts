...
function createClient() {
  ...
  ws.on("open", () => {
    ...
    const query =
      "Create a campaign for men over 50 years old, with a discount of 20% in all products. Include an email explaining the discount.";
  ...
  ws.on("message", (message: string) => {
    ...
    if (data.type === "tool") {
      ...
      if (
        ...
        ws.send(
          JSON.stringify({ type: "toolResponse", response: result.toString() })
        );
      } else {
      ...
      console.warn(
        "This simulator script was supossed to know how to answer the petition automatically, but it doesn't know how to process this."
      );
    }
  });
